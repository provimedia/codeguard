...
6. **Report through the cockpit.** At phase boundaries (intake, mode entry, step completion, verdict) render the matching template from `references/output-style.md` — visuals replace the prose report they frame, never duplicate it; question/trivial tasks stay plain. Read the sheet once per session before the first render.
