...
- **v16.2** (2026-07) — Cockpit Edition. Canonical visual output layer `references/output-style.md`: intake card, mode banners, checklist trackers with 20-cell progress bars, worklist pulse, lamp-based verdict blocks — rendered at phase boundaries only, replacing (never duplicating) the prose reports; all binding content rules unchanged. No ANSI (markdown renderer), open ━ frames only, clistatus-derived alignment laws.
